...
<body>
    <div id="container">
        ...
        <div id="header-detail">
            ...
            <span style="margin-left:-100px">LEVEL 2 SCOR SOURCE<br>sD3.15 INVOICE</span>
        </div>
        ...
        <?php
            ...
            for ($i = 0; $i < count($makeArray); $i++){
                $jumlahAktual += $makeArray[$i]['InvoicesFaultless'];
                $jumlahTarget += $makeArray[$i]['InvoicesFault'];
            }
        ...
        <div id="attribute">
                ...
                <div id="attribute-title" style="margin-left:2px">
                    <table style="border:0px;border-collapse: collapse; width: 100%; height: 50px">
                        <tr>
                            ...
                            <td style="background: #d9d9d9; padding: 5px;"><b>KPI: </b> % of faultless invoices</td>
                        </tr>
                    </table>
                </div>
                ...
                <div id="attribute-left-top">
                    ...
                    <span style="float:left; text-align:center; margin-left:15px;margin-top:90px; width:40%;">
                        <table style="text-align:center; font-size:18px">
                            <tr>
                                <td colspan="2" style="color:white">Last Quarter percentage of faultless invoices</td>
                            </tr>
                            <tr style="font-weight:bold">
                            <tr style="font-weight:bold">
                                <td style="color:#A5B592">Faultless<br><?php echo number_format(($jumlahAktual/($jumlahAktual + $jumlahTarget) *100), 1) ?>%</td>
                                <td style="color:#F3A447">Fault<br><?php echo number_format(($jumlahTarget/($jumlahAktual + $jumlahTarget) *100), 1) ?>%</td>
                            </tr>
                        </table>
                    </span>
                </div>
            ...
            <div id="attribute-right">
                ...
                <div id="attribute-right-bottom" style="margin-right:2px">
                    <table style="">
                        <tr>
                            <td style="padding: 5px; border-right: solid 2px white;">Total percentage of faultless invoices for last 16 quarter<br><?php echo number_format($jumlahAktual, 0, ",", ".") ?></td>
                            <td style="padding: 5px;">Total percentage of fault invoices for last 16 quarter<br><?php echo number_format($jumlahTarget, 0, ",", ".") ?></td>
                        </tr>
                    </table>
                </div>
...
<script type="text/javascript">
    ...
    function drawChart() {
        var dataResponsivenessLeftTop = google.visualization.arrayToDataTable([
            ['Task', 'Hours per Day'],
            ['Faultless',     <?php echo $jumlahAktual ?>],
            ['Fault',      <?php echo $jumlahTarget ?>]
        ]);
...
<script type="text/javascript">
    ...
    function drawStacked() {
        var dataResponsivenessLeftBottom = google.visualization.arrayToDataTable([
            ['', 'Average faultless invoices for last 16 quarter', 'Average fault invoices for last 16 quarter'],
...
<script type="text/javascript">
    ...
    function drawColColors() {
        ...
        data.addColumn('number', 'Faultless');
        data.addColumn('number', 'Fault');

        data.addRows([
            <?php
            for ($i = count($makeArray)-1; $i >= 0; $i--){
                $order = $makeArray[$i];
                echo "['Q".$order['Quarter']."-".trim($order['Year'],'20')."', ".$order['InvoicesFaultless'].", ".$order['InvoicesFault']."],";
            }
            ?>
        ]);
